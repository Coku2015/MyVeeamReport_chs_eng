...
<#
    ...
    Requires:
    Veeam Backup & Replication v12 (full or console install)
...
Write-Host "# 请确保您的机器能够访问VBR Console                        #"
...
# Version 12.0.0 - LW
# Chinese localized with v12 updates
...
#$allJobs2Rp = @($allJobs | Where-Object{$_.JobType -eq "Replica"})
$allJobsRp = @($allJobs | Where-Object{$_.TypeToString -eq "VMware Replication"})
# Get all Backup Copy Jobs
$allJobsBc = @($allJobs | Where-Object{$_.TypeToString -eq "VMware Backup Copy"})
# Get all Agent Backup Jobs
$allJobsEp = @($allJobs | Where-Object{$_.JobType -eq "EpAgentBackup"})
# Get all NAS Backup Jobs
$allJobsnas = @($allJobs | Where-Object{$_.JobType -eq "NASBackup"})
# Get all Tape Jobs
$allJobsTp = @()
...
# Get all SAP Jobs
$allJobsSAP = @($allJobsPlugin | Where-Object{$_.PluginType -eq "SAP"})

# Get all RMAN Jobs
$allJobsRMAN = @($allJobsPlugin | Where-Object{$_.PluginType -eq "RMAN"})
...
$backupsEp = @($jobBackups | Where-Object{$_.JobType -eq "EpAgentManagement"})
...
$sessListNAS = @($allSess | Where-Object{($_.EndTime -ge (Get-Date).AddHours(-$HourstoCheck) -or $_.CreationTime -ge (Get-Date).AddHours(-$HourstoCheck) -or $_.State -eq "Working") -and $_.JobType -eq "NasBackup"})
If ($null -ne $nasJob -and $nasJob -ne "") {
  $allJobsnasTmp = @()
  $sessListnasTmp = @()
  $backupsnasTmp = @()
  Foreach ($nJob in $nasJob) {
    $allJobsnasTmp += $allJobsnas | Where-Object{$_.Name -like $nJob}
    $sessListnasTmp += $sessListnas | Where-Object{$_.JobName -like $nJob}
    $backupsnasTmp += $backupsnas | Where-Object{$_.JobName -like $nJob}
  }
  $allJobsnas = $allJobsnasTmp | Sort-Object Id -Unique
  $sessListnas = $sessListnasTmp | Sort-Object Id -Unique
  $backupsnas = $backupsnasTmp | Sort-Object Id -Unique
}
If ($onlyLastnas) {
  $tempSessListnas = $sessListnas
  $sessListnas = @()
  Foreach($job in $allJobsnas) {
    $sessListnas += $tempSessListnas | Where-Object{$_.Jobname -eq $job.name} | Sort-Object EndTime -Descending | Select-Object -First 1
  }
}
...
$totalXfernas = 0
$totalReadnas = 0
$sessListNAS | ForEach-Object{$totalXfernas += $([Math]::Round([Decimal]$_.Progress.TransferedSize/1GB, 2))}
$sessListNAS | ForEach-Object{$totalReadnas += $([Math]::Round([Decimal]$_.Progress.ReadSize/1GB, 2))}
$successSessionsnas = @($sessListNAS | Where-Object{$_.Result -eq "Success"})
$warningSessionsnas = @($sessListNAS | Where-Object{$_.Result -eq "Warning"})
$failsSessionsnas = @($sessListNAS | Where-Object{$_.Result -eq "Failed"})
$runningSessionsnas = @($sessListNAS | Where-Object{$_.State -eq "Working"})
$failedSessionsnas = @($sessListNAS | Where-Object{($_.Result -eq "Failed") -and ($_.WillBeRetried -ne "True")})

# Gather all SAPBackup Sessions within timeframe
$sessListSAP = @($SAPSessions | Where-Object{($_.EndTime -ge (Get-Date).AddHours(-$HourstoCheck) -or $_.CreationTime -ge (Get-Date).AddHours(-$HourstoCheck) -or $_.State -eq "Working")})
If ($onlyLastSAP) {
  $tempSessListSAP = $sessListSAP
  $sessListSAP = @()
  Foreach($job in $allJobsSAP) {
    $sessListSAP += $tempSessListSAP | Where-Object{$_.JobId -eq $job.Id} | Sort-Object EndTime -Descending | Select-Object -First 1
  }
}
...
$totalXferSAP = 0
$totalReadSAP = 0
$sessListSAP | ForEach-Object{$totalXferSAP += $([Math]::Round([Decimal]$_.Progress.TransferedSize/1GB, 2))}
$sessListSAP | ForEach-Object{$totalReadSAP += $([Math]::Round([Decimal]$_.Progress.ReadSize/1GB, 2))}
$successSessionsSAP = @($sessListSAP | Where-Object{$_.Result -eq "Success"})
$warningSessionsSAP = @($sessListSAP | Where-Object{$_.Result -eq "Warning"})
$failsSessionsSAP = @($sessListSAP | Where-Object{$_.Result -eq "Failed"})
$runningSessionsSAP = @($sessListSAP | Where-Object{$_.State -eq "Working"})
$failedSessionsSAP = @($sessListSAP | Where-Object{($_.Result -eq "Failed") -and ($_.WillBeRetried -ne "True")})

# Gather all RMANBackup Sessions within timeframe
$sessListRMAN = @($RMANSessions | Where-Object{($_.EndTime -ge (Get-Date).AddHours(-$HourstoCheck) -or $_.CreationTime -ge (Get-Date).AddHours(-$HourstoCheck) -or $_.State -eq "Working")})
If ($onlyLastRMAN) {
  $tempSessListRMAN = $sessListRMAN
  $sessListRMAN = @()
  Foreach($job in $allJobsRMAN) {
    $sessListRMAN += $tempSessListRMAN | Where-Object{$_.JobId -eq $job.Id} | Sort-Object EndTime -Descending | Select-Object -First 1
  }
}
...
$totalXferRMAN = 0
$totalReadRMAN = 0
$sessListRMAN | ForEach-Object{$totalXferRMAN += $([Math]::Round([Decimal]$_.Progress.TransferedSize/1GB, 2))}
$sessListRMAN | ForEach-Object{$totalReadRMAN += $([Math]::Round([Decimal]$_.Progress.ReadSize/1GB, 2))}
$successSessionsRMAN = @($sessListRMAN | Where-Object{$_.Result -eq "Success"})
$warningSessionsRMAN = @($sessListRMAN | Where-Object{$_.Result -eq "Warning"})
$failsSessionsRMAN = @($sessListRMAN | Where-Object{$_.Result -eq "Failed"})
$runningSessionsRMAN = @($sessListRMAN | Where-Object{$_.State -eq "Working"})
$failedSessionsRMAN = @($sessListRMAN | Where-Object{($_.Result -eq "Failed") -and ($_.WillBeRetried -ne "True")})
  ...
  Process {
    Foreach ($r in $Repository) {
      ...
      $rType = switch ($r.Type) {
        ...
        "AmazonS3" {"AmazonS3"}
        "AmazonS3Compatible" {"S3Compatible"}
        default {"Unknown"}   
      }
...
Function Get-VeeamSupportDate {
  ...
  Try{
    ...
    $veeamLicInfo = [string]::Join($null, ($regBinary | ForEach-Object { [char][int]$_; }))
    # Convert Binary key
...
If ($showJobsBk) {
  If ($allJobsBk.count -gt 0) {
    ...
    If ($allJobsBk.Result -match "Failed") {
      $allJobsBkHead = $subHead01err
    } ElseIf ($allJobsBk.Result -match "Warning") {
      $allJobsBkHead = $subHead01war
    } ElseIf ($allJobsBk.Result -match "Success") {
      $allJobsBkHead = $subHead01suc
    } Else {
      $allJobsBkHead = $subHead01
    } 
    $bodyJobsBk = $allJobsBkHead + "Backup Job Status" + $subHead02 + $bodyJobsBk
  }
}
...
If ($showAllTasksBk) {
  If ($taskListBk.count -gt 0) {
    ...
    Foreach($taskBk in $taskListBk){
      $arrAllTasksBk += $taskBk | Select-Object @{Name="VM Name"; Expression = {$_.Name}},
        @{Name="Job Name"; Expression = {$_.JobSess.Name}},
        @{Name="Repository"; Expression = {
          If ($($repoList | Where-Object {$_.Id -eq $taskBk.Info.WorkDetails.RepositoryId}).Name) {
            $($repoList | Where-Object {$_.Id -eq $taskBk.Info.WorkDetails.RepositoryId}).Name
          } Else {
            $($repoListSo | Where-Object {$_.Id -eq $taskBk.Info.WorkDetails.RepositoryId}).Name
          }
        }},
...
If ($showJobsEp) {
  If ($allJobsEp.count -gt 0) {
    ...
    Foreach($epJob in $allJobsEp) {
        ...
        $bodyJobsEp += $epJob | Select-Object @{Name="Job Name"; Expression = {$_.Name}},
        @{Name="Description"; Expression = {$_.Description}},
        @{Name="Enabled"; Expression = {$($allJobsEpadd | Where-Object {$_.Id -eq $epJob.Id}).JobEnabled}},
        @{Name="Status"; Expression = {
          If ($epJob.IsRunning) {
            $currentSess = $runningSessionsEp | Where-Object{$_.JobName -eq $epJob.Name}
            $csessPercent = $currentSess.Progress.Percents
            $csessSpeed = [Math]::Round($currentSess.Progress.AvgSpeed/1MB,2)
            $cStatus = "$($csessPercent)% completed at $($csessSpeed) MB/s"
            $cStatus
        ...
        @{Name="Target Repo"; Expression = {$($allJobsEpadd | Where-Object {$_.Id -eq $epJob.Id}).BackupRepository.Name}},
        @{Name="Next Run"; Expression = {
          If ($_.IsScheduleEnabled -eq $false) {"<Disabled>"}
          ElseIf ($_.Options.JobOptions.RunManually) {"<not scheduled>"}
          ElseIf ($_.ScheduleOptions.IsContinious) {"<Continious>"}
          ElseIf ($_.ScheduleOptions.OptionsScheduleAfterJob.IsEnabled) {"After [" + $(($allJobs + $allJobsTp) | Where-Object {$_.Id -eq $epJob.Info.ParentScheduleId}).Name + "]"}
          Else {$epjobso.NextRun}
        }},
        @{Name="Last Result"; Expression = {If ($_.Info.LatestStatus -eq "None"){""}Else{$_.Info.LatestStatus}}}
        }        
    $bodyJobsEp = $bodyJobsEp | Sort-Object "Name" | ConvertTo-HTML -Fragment
...
If ($showBackupSizeEp) {
  If ($backupsEp.count -gt 0) {
    $bodyJobSizeEp = Get-BackupSize -backups $backupsEp | Sort-Object JobName | Select-Object @{Name="Job Name - Machine Name"; Expression = {$_.JobName}},
...
If ($showAllTasksEP) {
  If ($taskListEP.count -gt 0) {
    ...
    Foreach($taskEP in $taskListEP){
      $arrAllTasksEP += $taskEP | Select-Object @{Name="Server Name"; Expression = {$_.Name}},
        @{Name="Job Name"; Expression = {$_.JobSess.Name}},
        @{Name="Repository"; Expression = {
          If ($($repoList | Where-Object {$_.Id -eq $taskEP.Info.WorkDetails.RepositoryId}).Name) {
            $($repoList | Where-Object {$_.Id -eq $taskEP.Info.WorkDetails.RepositoryId}).Name
          } Else {
            $($repoListSo | Where-Object {$_.Id -eq $taskEP.Info.WorkDetails.RepositoryId}).Name
          }
        }},
        @{Name="Start Time"; Expression = {$_.Progress.StartTimeLocal}},
        @{Name="Stop Time"; Expression = {If ($_.Progress.StopTimeLocal -eq "1/1/1900 12:00:00 AM"){"-"} Else {$_.Progress.StopTimeLocal}}},
...
If ($showSummarynas) {
  ...
  $arrSummarynas =  $vbrMasterObj | Select-Object @{Name=$total; Expression = {$_.Sessions}},
    @{Name="Read (GB)"; Expression = {$_.Read}}, @{Name="Transferred (GB)"; Expression = {$_.Transferred}},
    @{Name="Running"; Expression = {$_.Running}}, @{Name="Successful"; Expression = {$_.Successful}},
    @{Name="Warnings"; Expression = {$_.Warning}},
    @{Name="Failed"; Expression = {$_.Failed}}
...
If ($showJobsnas) {
  If ($allJobsnas.count -gt 0) {
    ...
    Foreach($nasJob in $allJobsnas) {
      ...
      $bodyJobsnas += $nasJob | Select-Object @{Name="Job Name"; Expression = {$_.Name}},
        @{Name="Enabled"; Expression = {$_.Info.IsScheduleEnabled}},
        @{Name="Status"; Expression = {
          If ($nasJob.IsRunning) {
            $currentSess = $runningSessionsnas | Where-Object{$_.JobName -eq $nasJob.Name}
            $csessPercent = $currentSess.Progress.Percents
            $csessSpeed = [Math]::Round($currentSess.Info.Progress.AvgSpeed/1MB,2)
            $cStatus = "$($csessPercent)% completed at $($csessSpeed) MB/s"
            $cStatus
        ...
        @{Name="Target Repo"; Expression = {
          If ($($repoList | Where-Object {$_.Id -eq $nasJob.Info.TargetRepositoryId}).Name) {$($repoList | Where-Object {$_.Id -eq $nasJob.Info.TargetRepositoryId}).Name}
          Else {$($repoListSo | Where-Object {$_.Id -eq $nasJob.Info.TargetRepositoryId}).Name}}},
        @{Name="Next Run"; Expression = {
          If ($_.IsScheduleEnabled -eq $false) {"<Disabled>"}
          ElseIf ($_.Options.JobOptions.RunManually) {"<not scheduled>"}
          ElseIf ($_.ScheduleOptions.IsContinious) {"<Continious>"}
          ElseIf ($_.ScheduleOptions.OptionsScheduleAfterJob.IsEnabled) {"After [" + $(($allJobs + $allJobsTp) | Where-Object {$_.Id -eq $nasJob.Info.ParentScheduleId}).Name + "]"}
          Else {$nasjobso.NextRun}}},
        @{Name="Last Result"; Expression = {If ($_.Info.LatestStatus -eq "None"){""}Else{$_.Info.LatestStatus}}}
    }
    $bodyJobsnas = $bodyJobsnas | Sort-Object "Next Run" | ConvertTo-HTML -Fragment
...
If ($showAllSessnas) {
  If ($sessListnas.count -gt 0) {
    If ($showDetailednas) {
      $arrAllSessnas = $sessListnas | Sort-Object Creationtime | Select-Object @{Name="Job Name"; Expression = {$_.Name}},
        @{Name="State"; Expression = {$_.State}},
        @{Name="Start Time"; Expression = {$_.CreationTime}},
        @{Name="Stop Time"; Expression = {If ($_.EndTime -eq "1/1/1900 12:00:00 AM"){"-"} Else {$_.EndTime}}},
        @{Name="Duration (HH:MM:SS)"; Expression = {Get-Duration -ts $_.Progress.Duration}},                    
...
If ($showRunningnas) {
  If ($runningSessionsnas.count -gt 0) {
    $bodyRunningnas = $runningSessionsnas | Sort-Object Creationtime | Select-Object @{Name="Job Name"; Expression = {$_.Name}},
      @{Name="Start Time"; Expression = {$_.CreationTime}},
      @{Name="Duration (HH:MM:SS)"; Expression = {Get-Duration -ts $_.Progress.Duration}},
      @{Name="Avg Speed (MB/s)"; Expression = {[Math]::Round($_.Info.Progress.AvgSpeed/1MB,2)}},
      @{Name="Read (GB)"; Expression = {[Math]::Round([Decimal]$_.Progress.ReadSize/1GB, 2)}},
...
If ($showWarnFailnas) {
  ...
  If ($sessWF.count -gt 0) {
    ...
    If ($showDetailednas) {
      $arrSessWFnas = $sessWF | Sort-Object Creationtime | Select-Object @{Name="Job Name"; Expression = {$_.Name}},
        @{Name="Start Time"; Expression = {$_.CreationTime}},
        @{Name="Stop Time"; Expression = {$_.EndTime}},
        @{Name="Duration (HH:MM:SS)"; Expression = {Get-Duration -ts $_.Progress.Duration}},                    
        @{Name="Avg Speed (MB/s)"; Expression = {[Math]::Round($_.Info.Progress.AvgSpeed/1MB,2)}},
...
If ($showSuccessnas) {
  If ($successSessionsnas.count -gt 0) {
    ...
    If ($showDetailednas) {
      $bodySessSuccnas = $successSessionsnas | Sort-Object Creationtime | Select-Object @{Name="Job Name"; Expression = {$_.Name}},
        @{Name="Start Time"; Expression = {$_.CreationTime}},
        @{Name="Stop Time"; Expression = {$_.EndTime}},
        @{Name="Duration (HH:MM:SS)"; Expression = {Get-Duration -ts $_.Progress.Duration}},
        @{Name="Avg Speed (MB/s)"; Expression = {[Math]::Round($_.Info.Progress.AvgSpeed/1MB,2)}},
...
If ($showSummarySAP) {
  ...
  $arrSummarySAP =  $vbrMasterObj | Select-Object @{Name=$total; Expression = {$_.Sessions}},
    @{Name="Read (GB)"; Expression = {$_.Read}}, @{Name="Transferred (GB)"; Expression = {$_.Transferred}},
    @{Name="Running"; Expression = {$_.Running}}, @{Name="Successful"; Expression = {$_.Successful}},
    @{Name="Warnings"; Expression = {$_.Warning}},
    @{Name="Failed"; Expression = {$_.Failed}}
...
If ($showJobsSAP) {
  If ($allJobsSAP.count -gt 0) {
    ...
    Foreach($SAPJob in $allJobsSAP) {
      $bodyJobsSAP += $SAPJob | Select-Object @{Name="Job Name"; Expression = {$_.Name}},
        @{Name="Enabled"; Expression = {$_.IsEnabled}},
        @{Name="Target Repo"; Expression = {
          If ($($repoList | Where-Object {$_.Id -eq $SAPJob.TargetRepositoryId}).Name) {$($repoList | Where-Object {$_.Id -eq $SAPJob.TargetRepositoryId}).Name}
          Else {$($repoListSo | Where-Object {$_.Id -eq $SAPJob.TargetRepositoryId}).Name}}},
        @{Name="Next Run"; Expression = {"<not scheduled>"}},
        @{Name="Last Result"; Expression = {If ($_.LatestStatus -eq "None"){""}Else{$_.LastResult}}}
    }
    $bodyJobsSAP = $bodyJobsSAP | Sort-Object "Job Name" | ConvertTo-HTML -Fragment
...
If ($showAllSessSAP) {
  If ($sessListSAP.count -gt 0) {
    If ($showDetailedSAP) {
      $arrAllSessSAP = $sessListSAP | Sort-Object Creationtime | Select-Object @{Name="Job Name"; Expression = {$_.Name}},
        @{Name="State"; Expression = {$_.State}},
        @{Name="Start Time"; Expression = {$_.CreationTime}},
        @{Name="Stop Time"; Expression = {If ($_.EndTime -eq "1/1/1900 12:00:00 AM"){"-"} Else {$_.EndTime}}},
        @{Name="Duration (HH:MM:SS)"; Expression = {Get-Duration -ts $_.Progress.Duration}},                    
...
If ($showRunningSAP) {
  If ($runningSessionsSAP.count -gt 0) {
    $bodyRunningSAP = $runningSessionsSAP | Sort-Object Creationtime | Select-Object @{Name="Job Name"; Expression = {$_.Name}},
      @{Name="Start Time"; Expression = {$_.CreationTime}},
      @{Name="Duration (HH:MM:SS)"; Expression = {Get-Duration -ts $_.Progress.Duration}},
      @{Name="Avg Speed (MB/s)"; Expression = {[Math]::Round($_.Info.Progress.AvgSpeed/1MB,2)}},
      @{Name="Read (GB)"; Expression = {[Math]::Round([Decimal]$_.Progress.ReadSize/1GB, 2)}},
...
If ($showWarnFailSAP) {
  ...
  If ($sessWF.count -gt 0) {
    ...
    If ($showDetailedSAP) {
      $arrSessWFSAP = $sessWF | Sort-Object Creationtime | Select-Object @{Name="Job Name"; Expression = {$_.Name}},
        @{Name="Start Time"; Expression = {$_.CreationTime}},
        @{Name="Stop Time"; Expression = {$_.EndTime}},
        @{Name="Duration (HH:MM:SS)"; Expression = {Get-Duration -ts $_.Progress.Duration}},                    
        @{Name="Avg Speed (MB/s)"; Expression = {[Math]::Round($_.Info.Progress.AvgSpeed/1MB,2)}},
...
If ($showSuccessSAP) {
  If ($successSessionsSAP.count -gt 0) {
    ...
    If ($showDetailedSAP) {
      $bodySessSuccSAP = $successSessionsSAP | Sort-Object Creationtime | Select-Object @{Name="Job Name"; Expression = {$_.Name}},
        @{Name="Start Time"; Expression = {$_.CreationTime}},
        @{Name="Stop Time"; Expression = {$_.EndTime}},
        @{Name="Duration (HH:MM:SS)"; Expression = {Get-Duration -ts $_.Progress.Duration}},
        @{Name="Avg Speed (MB/s)"; Expression = {[Math]::Round($_.Info.Progress.AvgSpeed/1MB,2)}},
...
If ($showSummaryRMAN) {
  ...
  $arrSummaryRMAN =  $vbrMasterObj | Select-Object @{Name=$total; Expression = {$_.Sessions}},
    @{Name="Read (GB)"; Expression = {$_.Read}}, @{Name="Transferred (GB)"; Expression = {$_.Transferred}},
    @{Name="Running"; Expression = {$_.Running}}, @{Name="Successful"; Expression = {$_.Successful}},
    @{Name="Warnings"; Expression = {$_.Warning}},
    @{Name="Failed"; Expression = {$_.Failed}}
...
If ($showJobsRMAN) {
  If ($allJobsRMAN.count -gt 0) {
    ...
    Foreach($RMANJob in $allJobsRMAN) {
      $bodyJobsRMAN += $RMANJob | Select-Object @{Name="Job Name"; Expression = {$_.Name}},
        @{Name="Enabled"; Expression = {$_.IsEnabled}},
        @{Name="Target Repo"; Expression = {
          If ($($repoList | Where-Object {$_.Id -eq $RMANJob.TargetRepositoryId}).Name) {$($repoList | Where-Object {$_.Id -eq $RMANJob.TargetRepositoryId}).Name}
          Else {$($repoListSo | Where-Object {$_.Id -eq $RMANJob.TargetRepositoryId}).Name}}},
        @{Name="Next Run"; Expression = {"<not scheduled>"}},
        @{Name="Last Result"; Expression = {If ($_.LatestStatus -eq "None"){""}Else{$_.LastResult}}}
    }
    $bodyJobsRMAN = $bodyJobsRMAN | Sort-Object "Job Name" | ConvertTo-HTML -Fragment
...
If ($showAllSessRMAN) {
  If ($sessListRMAN.count -gt 0) {
    If ($showDetailedRMAN) {
      $arrAllSessRMAN = $sessListRMAN | Sort-Object Creationtime | Select-Object @{Name="Job Name"; Expression = {$_.Name}},
        @{Name="State"; Expression = {$_.State}},
        @{Name="Start Time"; Expression = {$_.CreationTime}},
        @{Name="Stop Time"; Expression = {If ($_.EndTime -eq "1/1/1900 12:00:00 AM"){"-"} Else {$_.EndTime}}},
        @{Name="Duration (HH:MM:SS)"; Expression = {Get-Duration -ts $_.Progress.Duration}},                    
...
If ($showRunningRMAN) {
  If ($runningSessionsRMAN.count -gt 0) {
    $bodyRunningRMAN = $runningSessionsRMAN | Sort-Object Creationtime | Select-Object @{Name="Job Name"; Expression = {$_.Name}},
      @{Name="Start Time"; Expression = {$_.CreationTime}},
      @{Name="Duration (HH:MM:SS)"; Expression = {Get-Duration -ts $_.Progress.Duration}},
      @{Name="Avg Speed (MB/s)"; Expression = {[Math]::Round($_.Info.Progress.AvgSpeed/1MB,2)}},
      @{Name="Read (GB)"; Expression = {[Math]::Round([Decimal]$_.Progress.ReadSize/1GB, 2)}},
...
If ($showWarnFailRMAN) {
  ...
  If ($sessWF.count -gt 0) {
    ...
    If ($showDetailedRMAN) {
      $arrSessWFRMAN = $sessWF | Sort-Object Creationtime | Select-Object @{Name="Job Name"; Expression = {$_.Name}},
        @{Name="Start Time"; Expression = {$_.CreationTime}},
        @{Name="Stop Time"; Expression = {$_.EndTime}},
        @{Name="Duration (HH:MM:SS)"; Expression = {Get-Duration -ts $_.Progress.Duration}},                    
        @{Name="Avg Speed (MB/s)"; Expression = {[Math]::Round($_.Info.Progress.AvgSpeed/1MB,2)}},
...
If ($showSuccessRMAN) {
  If ($successSessionsRMAN.count -gt 0) {
    ...
    If ($showDetailedRMAN) {
      $bodySessSuccRMAN = $successSessionsRMAN | Sort-Object Creationtime | Select-Object @{Name="Job Name"; Expression = {$_.Name}},
        @{Name="Start Time"; Expression = {$_.CreationTime}},
        @{Name="Stop Time"; Expression = {$_.EndTime}},
        @{Name="Duration (HH:MM:SS)"; Expression = {Get-Duration -ts $_.Progress.Duration}},
        @{Name="Avg Speed (MB/s)"; Expression = {[Math]::Round($_.Info.Progress.AvgSpeed/1MB,2)}},
  ...
  If ($allJobsSb.count -gt 0) {
    ...
    Foreach($SbJob in $allJobsSb) {
      $bodyJobsSb += $SbJob | Select-Object @{Name="Job Name"; Expression = {$_.Name}},
        @{Name="Enabled"; Expression = {$_.ScheduleEnabled}},
        @{Name="Status"; Expression = {
        ...
        @{Name="Application Group"; Expression = {$_.ApplicationGroup}},
        @{Name="Virtual Lab"; Expression = {$_.VirtualLab}},
        @{Name="Linked Jobs"; Expression = {$($_.GetLinkedJobs()).Name -join ","}},
        @{Name="Next Run"; Expression = {$_.NextRun}},
        @{Name="Last Result"; Expression = {$_.LastResult}}
    }
...
$taskListSb = @()
$taskListSb += $sessListSb | Get-VBRSureBackupTaskSession
...
$runningTasksSb = @()
$runningTasksSb += $runningSessionsSb | Get-VBRSureBackupTaskSession | Where-Object{$_.Status -ne "Stopped"}
  ...
  If ($null -ne $repoList) {
    $arrRepo = $repoList | Get-VBRRepoInfo | Select-Object @{Name="Repository Name"; Expression = {$_.Target}},
      ...
      @{Name="Status"; Expression = {
        ...
        elseif ($_.rType -eq "AmazonS3") {"OK"}
        elseif ($_.rType -eq "S3Compatible") {"OK"}
        elseif ($_.rType -eq "AzureStorage") {"OK"}
        elseif ($_.rType -eq "WasabiS3") {"OK"}
